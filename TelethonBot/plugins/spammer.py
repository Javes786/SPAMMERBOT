  ...
  if not e.text in abcd:
        ...
        if LOGGER_GROUP:
            await e.client.send_message(
                LOGGER_GROUP,
                "#BIGSPAM\n"
                        + f"BiGSPaM was executed successfully in (await {e.get_chat.title()})(`{e.chat_id}`) with {counter} times with {e.text}",
                    )
  ...
  if not e.text in abcd:
        ...
        if LOGGER_GROUP:
            await e.client.send_message(
                LOGGER_GROUP,
                "#SPAM\n"
                        + f"SPaM was executed successfully in (await {e.get_chat.title()})(`{e.chat_id}`) with {counter} times with {e.text}",
                    )
